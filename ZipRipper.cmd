...
IF !WORDLIST! EQU 1 (
IF !ONLINE! EQU 1 (
SET "LMSG=Cyclone wordlist selected - The wordlist will be included in the resume data"
) ELSE (
SET "LMSG=Internet connection unavailable - Automatic retrieval of Cyclone wordlist is not possible. Default and Custom wordlist options are available."
CALL :LISTMESSAGE "!LMSG!"
CALL :RESETWORDLIST
GOTO :MAIN
)
)
IF !WORDLIST! EQU 2 SET "LMSG=Custom wordlist selected - The wordlist will be included in the resume data - Please select a wordlist file"
IF !WORDLIST! GEQ 1 (
CALL :LISTMESSAGE "!LMSG!"
CALL :GETWORDLIST LISTNAME
IF NOT !LISTNAME!=="" (
...
SETLOCAL ENABLEDELAYEDEXPANSION
IF DEFINED LISTNAME >nul 2>&1 MOVE /Y !LISTNAME! "%ProgramData%\JtR\run\password.lst"
...
:RESUMEDECIDE
FOR /F "usebackq tokens=* delims=" %%# IN (`POWERSHELL -nop -c "$^=New-Object -ComObject Wscript.Shell;$^.Popup('Click OK to resume using the saved session data/wordlist, or Cancel to remove the saved job and start over',0,'There is a job in progress for this file!',0x1)"`) DO SET %1=%%#
EXIT /b
...
IF EXIST "%ProgramData%\cyclone_hk.txt.7z" "%ProgramData%\7zr.exe" x -y "%ProgramData%\cyclone_hk.txt.7z" -o"%ProgramData%\">nul
...
:DISPLAYINFOB
POWERSHELL -nop -c "$^={$Notify=[PowerShell]::Create().AddScript({$Audio=New-Object System.Media.SoundPlayer;$Audio.SoundLocation=$env:WinDir + '\Media\Windows Notify System Generic.wav';$Audio.playsync()});$rs=[RunspaceFactory]::CreateRunspace();$runspace.ApartmentState="^""STA"^"";$rs.ThreadOptions="^""ReuseThread"^"";$rs.Open();$Notify.Runspace=$rs;$Notify.BeginInvoke()};&$^;$Msg=@();$Msg+="^""[ZIP-Ripper] - FOUND PASSWORDS`n"^"";$Msg+="^"" %DATE% + %TIME%`n"^"";$Msg+="^""==============================`n"^"";$Msg+="^""`n"^"";$Msg+="^""TOO MANY TO LIST`n"^"";$Msg+="^""`n"^"";$Msg+="^""==============================`n"^"";$Msg+="^""Save Location:`n"^"";$Msg+="^"""^"""^""%UserProfile%\Desktop\ZipRipper-Passwords.txt"^"""^"""^"";$PopUp=New-Object -ComObject Wscript.Shell;$PopUp.Popup($Msg,0,'Message from ZIP-Ripper',0x0)">nul
EXIT /b
