...
IF !WORDLIST! EQU 1 SET "LMSG=RockYou wordlist selected - The wordlist will be included in the resume data. If you need to resume this session you can select the file with the default settings and this wordlist will be used"
IF !WORDLIST! EQU 2 SET "LMSG=Custom wordlist selected - The wordlist will be included in the resume data. If you need to resume this session you can select the file with the default settings and this wordlist will be used - Please select a wordlist file"
IF !WORDLIST! GEQ 1 (
POWERSHELL -nop -c "$^={$Notify=[PowerShell]::Create().AddScript({$Audio=New-Object System.Media.SoundPlayer;$Audio.SoundLocation=$env:WinDir + '\Media\Windows Notify System Generic.wav';$Audio.playsync()});$rs=[RunspaceFactory]::CreateRunspace();$rs.ApartmentState="^""STA"^"";$rs.ThreadOptions="^""ReuseThread"^"";$rs.Open();$Notify.Runspace=$rs;$Notify.BeginInvoke()};&$^;$PopUp=New-Object -ComObject Wscript.Shell;$PopUp.Popup("^""!LMSG!"^"",0,'INFO:',0 + 64)">nul&CALL :GETWORDLIST LISTNAME
