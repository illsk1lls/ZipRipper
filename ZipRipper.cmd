@ECHO OFF
::Spaces must not be present in front of powershell commands
...
::Supported extensions and dependencies
SET "NATIVE=ZIP,RAR"
SET "PERL=7z,PDF"
::Init vars
SET/A GPU=0
...
::Check if a supported extension was dropped and flag for dependencies
IF "%~1"=="" (
...
CD /D %~dp0
::Copy to and run from %ProgramData% if not - include zr-offline.txt if present
...
TITLE %TitleName%
::Center CMD window with powershell, doesnt work with new terminal
...
::Test internet connection, if FALSE exit if zr-offline.txt is not present
...
::Remove old incomplete BITS downloads from previous interrupted sessions, if present
...
CLS
SET "FLAG="
::If filesize is large hash will take a while
IF %~z1 GEQ 200000000 (
...
john pwhash !FLAG!
ENDLOCAL
::Check for found passwords
CALL :GETSIZE john.pot POTSIZE
::Build password list if found
SETLOCAL ENABLEDELAYEDEXPANSION
IF !POTSIZE! GEQ 1 (
	ENDLOCAL
...
::Due to MSG box limits, displayed text must be less than ~1600 bytes
...
:GETJTRREADY
CLS
::Check if zr-offline.txt is present, if not, run in online mode
IF NOT EXIST "%~dp0zr-offline.txt" (
...
POWERSHELL -nop -c "Invoke-WebRequest -Uri https://www.7-zip.org/a/7zr.exe -o '%~dp07zr.exe'";"Invoke-WebRequest -Uri https://www.7-zip.org/a/7z2300-extra.7z -o '%~dp07zExtra.7z'"
	CLS
	IF %ISPERL% EQU 1 (
::Download JtR, and perl portable
...
::Extract perl portable if needed
IF %ISPERL% EQU 1 (
	CLS
...
::Detect GPU lineup and OpenCL availability
...
::Delayed expansion required to retrieve returnvar value
SET/A "%2=%~z1"
EXIT/b
...
:MULTI
::Show multiple found passwords via hash matches between initial 'pwhash' and 'john.pot'
