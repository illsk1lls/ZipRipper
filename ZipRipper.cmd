...
CD /D %~dp0&IF NOT "%~f0" EQU "%ProgramData%\%~nx0" >nul 2>&1 COPY /Y "%~f0" "%ProgramData%"&(IF EXIST "%~dp0.zr-offline.txt" >nul 2>&1 COPY /Y "%~dp0.zr-offline.txt" "%ProgramData%")&START "" ""%ProgramData%\%~nx0"" "%_%">nul&EXIT/b
...
:GETJTRREADY
CLS&IF NOT EXIST "%~dp0.zr-offline.txt" (
...
) ELSE (ECHO Offline mode enabled, preparing resources...&REN "%~dp0.zr-offline.txt" .resources.exe&.resources -y -pDependencies>nul&>nul 2>&1 DEL .resources.exe /F /Q)
